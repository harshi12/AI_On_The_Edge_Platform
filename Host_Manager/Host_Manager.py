...
class host_manager:
	...
	def Model_Deploy(self, IP_username_password):
		...
		for ip in IPs:
			...
			command += "echo \"verma\" | sshfs red@192.168.31.29:/Users/red/Documents/nfs_server" + Model_Link + " ~/" + str(self.App["App_ID"])+"/Models -o password_stdin"
			file.write(command)
			file.close()
			time.sleep(5)
		...
		for service in Services:
			if service["Criticality"] == "Low":
				...
				for i in range(No_Instances):
					...
					command = "mkdir -p ~/" + str(self.App["AppDev_ID"]) + "/" + str(self.App["App_ID"]) + "/Services\n"
					command += "echo " + IP_username_password[IPs[j]][1] + " | sudo -S apt-get install sshpass\n"
					#command += "sshpass -p \"verma\" ssh red@192.168.31.29 \"exit\"\n"
					command += "sshpass -p \"Akanksha21!\" ssh akanksha@192.168.31.244 \"exit\"\n"
					...
					command += "echo \"Akanksha21!\" | sshfs akanksha@192.168.31.244:/mnt/Repository" + Service_Link +" ~/" + str(self.App["AppDev_ID"]) + "/" + str(self.App["App_ID"]) + "/Services -o password_stdin"
					file.write(command)
					file.close()
					time.sleep(5)
...
def callback_DM(ch, method, properties, body):
	...
	if Request_Type == "Model_Submit":
		...
		model = hm_obj.Model_Deploy(IP_username_password)
		#obj1 = RabbitMQ()
