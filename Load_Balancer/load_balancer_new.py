...
def handle_host_manager_request(number_of_instances):
    ...
    while k<number_of_instances:
        if pool[list(pool)[itr]] == 'available':
            ...
            response[list(pool)[itr]]=[host_credentials[list(pool)[itr]]["Username"],host_credentials[list(pool)[itr]]["Password"]]
            # pool[list(pool)[itr]] = 'allotted'
            itr += 1
            k += 1
        else:
            itr += 1
        if itr >= len(pool) and k < number_of_instances:
            itr -= 1
            while k < number_of_instances:
                response[list(pool)[itr]]=[host_credentials[list(pool)[itr]]["Username"],host_credentials[list(pool)[itr]]["Password"]]
                k += 1
    ...
    for i in rev_pool_list:
        if rev_pool_list[i]>=upper_threshold:
            ...
            for each_service in list_of_services_to_be_migrated:
                ...
                free_ip = [free_ip]
                request_MT = {"Request_Type": "Gateway_Deploy","Service_ID" : each_service,"Gateway_IPs":free_ip}
...
def service_upscaling():
    ...
    for service_id in service_ids:
        ...
        print("Queue length of "+queue_name+"  "+str(length_of_queue))
